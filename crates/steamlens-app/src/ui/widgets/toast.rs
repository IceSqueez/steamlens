...
use crate::theme::{C_TEXT_MUTED, C_TEXT_PRIMARY};
...
impl<'a, M: 'a + Clone> From<Toast<'a, M>> for Element<'a, M> {
    fn from(t: Toast<'a, M>) -> Self {
        ...
        let mut info_col = column![text(t.title).size(12).color(C_TEXT_PRIMARY)].spacing(1);
        if let Some(body) = t.body {
            info_col = info_col.push(text(body).size(10).color(C_TEXT_MUTED));
        }
        ...
        let inner = container(content_row)
            ...
            .style(move |_: &iced::Theme| container::Style {
                background: Some(Background::Color(TOAST_SURFACE)),
                border: Border {
...
fn link_button<'a, M: 'a + Clone>(label: impl Into<Cow<'a, str>>, msg: M) -> Element<'a, M> {
    ...
    button(text(label).size(10).color(C_TEXT_MUTED))
        .on_press(msg)
