...
use crate::ffi::interfaces::ISteamUtils005;
...
impl Utils {
    pub(super) fn get_image(&self, handle: i32) -> Result<Option<Image>, SteamError> {
        ...
        // SAFETY: `steam_utils` was obtained from `GetISteamUtils("SteamUtils005")`;
        // the pipe is alive for the lifetime of the owning `Client`; Steam writes
        // through the stack pointers only on returning `true`; SysV-x64 ABI.
        let size_ok = unsafe {
    ...
    pub(super) fn poll_call_result(
        ...
        // SAFETY: `steam_utils` is live; Steam writes through the stack `failed`
        // pointer on completion-with-IO-error; SysV-x64 ABI.
